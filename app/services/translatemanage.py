...
async def translate_(document_path, service, session, correlation_id):
    try:
        ...
        if (service == "google") or (service == "deepl"):
            ...
            result = load_prompt_path(f"{TRANSLATE_PROMPT}.txt", session=session, correlation_id=correlation_id)
            cmd = ["pdf2zh", f"{document_path}", "-s", f"{service}", "-li", "EN", "-lo", "KO", "--output", f"{DOCS_SAVE_PATH}", "--prompt", f"{result.data['prompt_path']}"]
